...
for OPTIM in ${OPTIMS[@]}; do
  OUT="mmul${OPTIM}"
  g++ ${CXX_FLAGS} ${OPTIM} ${MAIN_FILES} ${SRC_FILES} -fopenmp -lgomp -I${INCLUDES} -o ${OUT}

  echo "Run ${OUT}"
  ./${OUT} | tee mmul${OPTIM}-openmp.json
  echo -e -n "\n"
done
