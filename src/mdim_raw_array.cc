...
/** デフォルトコンストラクタ */
MDimRawArray::MDimRawArray() {}

/**
 * 初期化する．
 */
void MDimRawArray::Init() {
  util::Fill(a_, 1.0);
  util::Fill(b_, 1.0);
  util::Fill(c_, 0.0);
}
 ...
 * @return タグ名
 */
std::string MDimRawArray::Tag() const {
  return "mdim_raw_array";
}
 ...
 * @return 実行結果．
 */
double MDimRawArray::GetResult(std::size_t i, std::size_t j) const {
  return c_[i][j];
}
